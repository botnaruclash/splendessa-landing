...
/* =========================================================
   Primary CTA magnetic pull (#book-call). Reads the cursor and
   writes a small translate on the button so it eases toward the
   pointer within a short radius; CSS eases the transform (0.13s)
   and returns it to rest. Throttled with a queued flag + rAF,
   same shape as the overlay scrubber. Self-contained: it only
   ever writes transform, never touches the click path — so the
   Cal.com wiring (initCal's once-listener + Cal's delegated
   data-cal-link handler on this same node) is untouched.
...
(function initMagneticCta() {
  ...
  const RADIUS = 80; // px beyond the button box where the pull begins
  const MAX = 7;     // px max displacement
  let px = 0;
  let py = 0;
  let queued = false;
  ...
  function apply() {
    ...
    if (Math.abs(dx) < r.width / 2 + RADIUS && Math.abs(dy) < r.height / 2 + RADIUS) {
      const nx = Math.max(-MAX, Math.min(MAX, dx * 0.3));
      const ny = Math.max(-MAX, Math.min(MAX, dy * 0.3));
      btn.style.transform = `translate(${nx.toFixed(1)}px, ${ny.toFixed(1)}px)`;
    } else if (btn.style.transform) {
      btn.style.transform = ""; // outside the radius → ease back to rest
    }
  }
  ...
  // Immediate reset when the pointer leaves the button itself.
